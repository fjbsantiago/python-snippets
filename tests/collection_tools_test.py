...
class TestBase(unittest.TestCase):
    def test_build_hierarchy(self):
        ...
        artist_nodes = collection_tools.build_hierarchy(artists, fields=['name', 'decade'])
        self.assertEquals(len(artist_nodes), 4)
        for node in artist_nodes:
            # Check that all names are in the first level
            self.assertTrue(node['title'] in ('kriss kross', 'buddy guy', 'muddy waters', 'rolling stones'))
